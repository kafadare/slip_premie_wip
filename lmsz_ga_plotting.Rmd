...
out_fans_folder <- glue("{csv_save_path}out_fans_slip/")
```
...
separate_plot_loop <- function(pheno_list, df_plot, out_fans_folder, x_breaks, x_labs){
  slip_plots <- list()
  lmsz_plots <- list()
  for (pheno_of_int in pheno_list) {
    #read in the csv for phenotype of interest
    out_fans_df <- read.csv(glue("{out_fans_folder}lmsz_centfans_ga_uncorrected_{pheno_of_int}.csv"))
    #Scale all phenotypes by 1000 and create sex_ga column
    out_fans_df <- out_fans_df %>% 
...
overlaid_plot_loop <- function(pheno_list, df_plot, out_fans_folder, x_breaks, x_labs){
