...
params:
  output_folder : "/mnt/isilon/bgdlab_processing/Eren/slip_premie_wip/output/mpr-allgrades-25.11/"
  centiles_path: "/mnt/isilon/bgdlab_processing/Eren/slip_premie_wip/SLIP_25_11/centile_csv/slip_mpr_centiles.csv"
...
```{r}
 ...
 participants %>%
     group_by(participant_id) %>%
     summarise(n_var = n_distinct(birth_weight_kg_qc)) %>%
     filter(n_var > 1)
  participants %>%
     group_by(participant_id) %>%
     summarise(n_var = n_distinct(ga_days_total_qc)) %>%
     filter(n_var > 1)
#
```
...
```{r}
vars_to_select <- c(full_vars, "participant_id", "session_id", "scan_id", "site", "sex", "adjusted_age_in_days", "avg_grade", "dx")
...
clean_centiles <- slip_centdf_clean(centiles, participants, qc_metrics, type = type, add_qc = add_qc_flag)
...
```{r}
##set bw > 1000kg to NA
bw_absurd <- sum(centiles_distinct$birth_weight_kg  > 10 | centiles_distinct$birth_weight_kg < 0.3, na.rm = TRUE)
print(paste0("WITHIN DISTINCT DF: Number of data points set to NA for recorded BW > 1000kg: ", bw_absurd))
centiles_distinct$birth_weight_kg[centiles_distinct$birth_weight_kg  > 10 | centiles_distinct$birth_weight_kg < 0.3] <- NA
##remove >4SDs
bw_mean <- mean(centiles_distinct$birth_weight_kg, na.rm = TRUE)
bw_sd <- sd(centiles_distinct$birth_weight_kg, na.rm = TRUE)
#apply QC function
centiles_distinct <- slip_bwga_qc(cent_df = centiles_distinct, bw_mean = bw_mean, bw_sd = bw_sd,
                                  bw_sd_limit = 4, ga_low_limit = 21, ga_high_limit = 42)
centiles_all <- slip_bwga_qc(cent_df = centiles_all, bw_mean = bw_mean, bw_sd = bw_sd, 
                             bw_sd_limit = 4, ga_low_limit = 22, ga_high_limit = 42)
```
...
# Weight/Height Centiles Calculation
```{r}
centiles_distinct$weight_at_enc_sds <- sds(value = centiles_distinct$weight_at_enc,
                                            age = centiles_distinct$adjusted_age_in_years,
                                            sex = centiles_distinct$sex,
                                            item = "weight",
                                            ref = who.ref,
                                            type = "SDS",
                                            male = "Male",
...
centiles_distinct$height_at_enc_sds <- sds(value = centiles_distinct$height_at_enc,
                                            age = centiles_distinct$adjusted_age_in_years,
                                            sex = centiles_distinct$sex,
                                            item = "height",
                                            ref = who.ref,
...
centiles_all$weight_at_enc_sds <- sds(value = centiles_all$weight_at_enc,
                                            age = centiles_all$adjusted_age_in_years,
...
centiles_distinct$weight_at_enc_cent <- sds(value = centiles_distinct$weight_at_enc,
                                            age = centiles_distinct$adjusted_age_in_years,
                                            sex = centiles_distinct$sex,
                                            item = "weight",
                                            ref = who.ref,
...
centiles_distinct$height_at_enc_cent <- sds(value = centiles_distinct$height_at_enc,
                                            age = centiles_distinct$adjusted_age_in_years,
                                            sex = centiles_distinct$sex,
                                            item = "height",
                                            ref = who.ref,
...
centiles_all$weight_at_enc_cent <- sds(value = centiles_all$weight_at_enc,
                                            age = centiles_all$adjusted_age_in_years,
                                            sex = centiles_all$sex,
                                            item = "weight",
                                            ref = who.ref,
...
centiles_all$height_at_enc_cent <- sds(value = centiles_all$height_at_enc,
                                            age = centiles_all$adjusted_age_in_years,
                                            sex = centiles_all$sex,
                                            item = "height",
                                            ref = who.ref,
...
centiles_distinct$weight_at_enc_nonadjage_sds <- sds(value = centiles_distinct$weight_at_enc,
                                            age = centiles_distinct$age_years,
                                            sex = centiles_distinct$sex,
                                            item = "weight",
                                            ref = who.ref,
                                            type = "SDS",
...
centiles_distinct$height_at_enc_nonadjage_sds <- sds(value = centiles_distinct$height_at_enc,
                                            age = centiles_distinct$age_years,
                                            sex = centiles_distinct$sex,
                                            item = "height",
                                            ref = who.ref,
...
centiles_all$weight_at_enc_nonadjage_sds <- sds(value = centiles_all$weight_at_enc,
                                            age = centiles_all$age_years,
                                            sex = centiles_all$sex,
                                            item = "weight",
                                            ref = who.ref,
                                            type = "SDS",
...
centiles_all$height_at_enc_nonadjage_sds <- sds(value = centiles_all$height_at_enc,
                                            age = centiles_all$age_years,
                                            sex = centiles_all$sex,
                                            item = "height",
                                            ref = who.ref,
...
centiles_distinct$weight_at_enc_nonadjage_cent <- sds(value = centiles_distinct$weight_at_enc,
                                            age = centiles_distinct$age_years,
                                            sex = centiles_distinct$sex,
                                            item = "weight",
                                            ref = who.ref,
...
centiles_distinct$height_at_enc_nonadjage_cent <- sds(value = centiles_distinct$height_at_enc,
                                            age = centiles_distinct$age_years,
                                            sex = centiles_distinct$sex,
                                            item = "height",
                                            ref = who.ref,
...
centiles_all$weight_at_enc_nonadjage_cent <- sds(value = centiles_all$weight_at_enc,
                                            age = centiles_all$age_years,
                                            sex = centiles_all$sex,
                                            item = "weight",
                                            ref = who.ref,
...
centiles_all$height_at_enc_nonadjage_cent <- sds(value = centiles_all$height_at_enc,
                                            age = centiles_all$age_years,
                                            sex = centiles_all$sex,
                                            item = "height",
                                            ref = who.ref,
