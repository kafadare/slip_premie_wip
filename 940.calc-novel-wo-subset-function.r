...
Calc.Novel <- function( NewData, Fit, BootFit=NULL, Apply=TRUE, NumberCores=1 ) {
    ...
    Cond.Base <- attr(Fit$param,"levels")[[lCOND]] # What is the baseline level for condition (should be CN)
    ...
    for( LAB in names(attr(Fit$param,"levels")) ) {
        if( LAB == lCOND ) { next } ## must skip the condition column, since within LOCAL there is only one level (healthy controls)
        if( !is.factor( NewData[,LAB] ) ) {stop("NewData column is not a factor when Fit object expects it to be")}
        
        if( LAB %in% names(attr(Fit$param,"contrasts")) ) { # extract contrast matrix
    ...
    # Get controls to estimate random effect on
