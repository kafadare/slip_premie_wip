...
participants_path <- glue("{slip_release_folder}BIDS/participants.tsv")
...
```{r}
vars_to_select <- c(all_vars, "participant_id", "session_id", "site", "sex", "age_days", "avg_grade", "dx")
...
centiles <- centiles %>% select(all_of(vars_to_select))
